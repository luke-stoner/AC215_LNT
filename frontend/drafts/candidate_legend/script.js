...
let impactWidth = 600 - impactMargin.left - impactMargin.right;
let impactHeight = 400 - impactMargin.top - impactMargin.bottom;
...
d3.csv("data/labeled.csv", row => {
    ...
    document.querySelectorAll('.candidate-container').forEach(container => {
        container.addEventListener('click', function() {
            ...
            updateCandidateSelection(this.id, this.classList.contains('selected'));
            updateVisualization(); // Existing function to redraw the chart
        });
    });

    let selectedCandidates = {
        "biden_button": false,
        "christie_button": false,
        "desantis_button": false,
        "haley_button": false,
        "kennedy_button": false,
        "pence_button": false,
        "ramaswamy_button": false,
        "scott_button": false,
        "trump_button": false,
        "williamson_button": false
    }

    document.querySelectorAll('.candidate_containers').forEach(container => {
        container.addEventListener('click', function() {
            // Toggle the selected status in the tracking object
            selectedCandidates[this.id] = !selectedCandidates[this.id];

            // Toggle the 'selected' class for visual feedback
            this.classList.toggle('selected');

            // Update the visualization based on the new selection
            updateVisualization();
        });
    });
    ...
    function updateVisualization() {
        ...
        const candidate_buttons = document.querySelectorAll('.candidate-container');
        // Iterate through checkboxes to find selected candidates
        candidate_buttons.forEach(candidate => {
    ...
    function initializeCandidateColors() {
        for (let candidateId in selectedCandidates) {
            let container = document.getElementById(candidateId);
            if (container) {
                // Set the initial color based on whether the candidate is selected
                container.style.backgroundColor = selectedCandidates[candidateId] ? getCandidateColor(candidateId) : 'grey';
            }
        }
    }

    function getCandidateColor(candidateId) {
        // Return the color associated with the candidate
        // This is an example, replace it with your actual color logic
        switch (candidateId) {
            case 'biden_button': return 'blue';
            case 'christie_button': return 'red';
            // ... other candidates
            default: return 'grey';
        }
    }
