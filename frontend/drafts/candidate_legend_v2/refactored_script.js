...
function initializeDashboard() {
    ...
    function updateVisualization() {
        ...
        let filtered_date_data = data.filter(d => d.date >= startDate && d.date <= endDate);
        ...
        lineGroups.selectAll('.datapoint')
            .on('mouseover', function(event, d) {
                ...
                var selectedMetric = document.getElementById("line-stat-selection-impact").value; // Get the selected metric's name
                ...
                tooltip.html(`<strong style='color: ${candidateColorMap[candidateName.toLowerCase()]};'>${candidateName}</strong><br/>${selectedMetric}: ${formattedDataValue}<br/>Date: ${formattedDate}`)
                    .style('opacity', 1)
        ...
        function highlightLine(candidateName) {
            ...
            svg.selectAll('.line-group').filter(d => d.candidate === candidateName)
                .selectAll('.line, .datapoint').classed('highlighted', true); // Highlight specific line and points
            // Add hovered class to the corresponding image
            let candidateNameLower = candidateName.toLowerCase();
            d3.select(`#${candidateNameLower}_button`).node().parentNode.classList.add('hovered'); // circle image
            timelineSvg.selectAll('.line').classed('dimmed', true);
            timelineSvg.selectAll('.line').filter(d => d.candidate === candidateName).classed('highlighted', true);
        }

        // Restore original styles on mouseout
        function restoreLines() {
            svg.selectAll('.line, .datapoint').classed('highlighted', false).classed('dimmed', false);
            ...
            timelineSvg.selectAll('.line').classed('highlighted', false).classed('dimmed', false);
        ...
        function bringToFront(candidateName) {
            svg.selectAll('.line-group').filter(d => d.candidate === candidateName).raise();
            timelineSvg.selectAll('.line').filter(d => d.candidate === candidateName).raise();

        }
        // Add mouseover and mouseout events to candidate images
        d3.selectAll('.candidate_containers img').on('mouseover', function() {
...
// Function to handle candidate image clicks
    ...
    function change_color(candidate, isSelected) {
        ...
        if (isSelected) {
            candidateButton.style("background-color", candidateColorMap[candidate.toLowerCase()])
    ...
    function addEventLine(eventDate, eventName) {
        ...
        svg.append('line')
            ...
            .attr('x2', eventX)
            .attr('y1', 0)
            .transition().duration(2000) // Transition for event line
            .attr('y2', height)
            .style('stroke', 'grey')
            .style('stroke-width', 1)
        ...
        svg.append('text')
            ...
            .transition().duration(1000) // Transition for label
...
// Call adjustTextSize when the page loads and when it's resized
    ...
    function updateDynamicTitle() {
        ...
        let eventSelectionText = eventSelect.options[eventSelect.selectedIndex].text;
        let lineStatSelectionText = lineStatSelect.options[lineStatSelect.selectedIndex].text;
    ...
    let width_timeline = pxls_leftcol - margin_timeline.left - margin_timeline.right;
    ...
    function updateTimelineVisualization(startDate, endDate, column) {
        ...
        let filteredCandidatesData = filtered_date_data_timeline.filter(d => candidateStatus[d.last_name.toLowerCase()]);
        timelineYScale = d3.scaleLinear().range([height_timeline, 0]);
        timelineYAxis = timelineSvg.append('g');
        // Then group this filtered data
        let groupedData_timeline = d3.group(filteredCandidatesData, d => d.last_name);
        ...
        if(column_timeline==='volume') {
            ...
            aggregatedData_timeline = Array.from(groupedData_timeline, ([key, values]) => {
                let weeklyData_timeline = d3.timeWeek.every(1).range(
                    d3.min(values, d => d.date),
                    d3.max(values, d => d.date)
                ).map(week => {
                    let filtered_timeline = values.filter(d => d.date >= week && d.date < d3.timeWeek.offset(week, 1));
                    ...
                    if (filtered_timeline.length === 0) {
                        ...
                        let count_timeline = d3.count(filtered_timeline, d => d.label) / 7;
        ...
        timelineYAxis.transition().duration(500).call(d3.axisLeft(timelineYScale).ticks(3));


        // Select and update circles for data points
        let points_timeline = timelineSvg.selectAll('.point-group')
            .data(aggregatedData_timeline);
        ...
        let newPoints_timeline = points_timeline.enter().append('g').attr('class', 'point-group');
        ...
        let circles_timeline = allPoints_timeline.selectAll('circle')
            .data(d => d.values);

        // I DONT WANT TO INCLUDE ANY CIRCLES
        circles_timeline.exit().remove(); // Remove circles that no longer have data
        let lines_timeline = timelineSvg.selectAll('.line').data(aggregatedData_timeline);
        ...
        rightOverlay.enter().append('rect')
